...
# Direct metric registry (function-based execution)
...
def handle_query(query: str, tables):
    ...
    by looking up the executable function in METRIC_REGISTRY_FUNCTIONS.
    """
    for metric_name, metric_fn in METRIC_REGISTRY_FUNCTIONS.items():
        if metric_name.lower() in query.lower():
            # Metric functions are expected to handle table selection internally
...
def answer_query(question, cleaned_tables, chart_gen):
    ...
    Question → Intent → Tables → Metric → Chart → Answer
    """
...
# -------------------------------------------------
# 3️⃣ Unified Router (ENTRY POINT)
# -------------------------------------------------
def route_query(question, cleaned_tables, chart_gen):
    """
    Unified query router:
    1) Try direct metric execution (fast, deterministic)
    2) Fallback to intent-based reasoning
    """

    # Attempt direct metric execution
    ...
    # Fallback to full reasoning pipeline
