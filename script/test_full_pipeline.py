...
def main():
    ...
    for t in cleaned_tables:
        try:
            index_dataframe_to_chroma(t["df"], source_name=t["source"], table_id=t["table_id"])
            total_rows += len(t["df"])
        except Exception as e:
            print(f"   ⚠️ Failed: {t['source']} table {t['table_id']}: {e}")
    ...
    # ==========================================
    print_section("4️⃣ TESTING RAG RETRIEVAL")
    queries = ["revenue growth financial data", "cash expenses burn rate", "profit margin operating income"]
    all_contexts = []
    for query in queries:
        try:
            contexts = retrieve_context(query, k=3)
    ...
    print_section("5️⃣ COMPUTING FINANCIAL METRICS")
    metrics_found = 0
    for t in cleaned_tables[:10]:
        metrics = compute_financial_metrics(t["df"])
        if metrics and metrics_found < 3:
            print(f"\n📊 {t['source']} (table {t['table_id']}):")
            for key, value in list(metrics.items())[:4]:
                print(f"   {key}: {value}")
            metrics_found += 1
    ...
    for t in cleaned_tables[:15]:
        ...
        for y_col in numeric_cols[:2]:
            ...
            output_path = chart_dir / f"{t['source']}_t{t['table_id']}.png"
            result = chart_gen_module.generate_chart(df=df, x_col=x_col, y_col=y_col, output_path=str(output_path))
            if result.get("image_path"):
                charts.append(result)
                chart_types[result.get("chart_type", "unknown")] += 1

    # ==========================================
    # 7️⃣ TEST CALCULATOR
    # ==========================================
    print_section("7️⃣ TESTING CALCULATOR")
    # ... (Tests 1-4 for calculator logic as defined in original script)
    ...
    for t in cleaned_tables[:5]:
        ...
        if trends: trends_found += 1

    # ==========================================
    # 8.5️⃣ QUERY ROUTER TEST
    # ==========================================
    ...
    try:
        ...
        result = compare_across_documents("compare revenue", cleaned_tables)
    except Exception as e:
        print(f"   ❌ Comparison Engine skipped: {e}")

    # ==========================================
    # 8.7️⃣ TESTING LLM NARRATION (GEMINI)
    # ==========================================
    print_section("8️⃣.7️⃣ TESTING LLM NARRATION (GEMINI)")
    try:
        from src.ai.llm_narrator import LLMNarrator
        
        print("\n🧠 Initializing Gemini narrator...")
        narrator = LLMNarrator()
        print("   ✅ Gemini connected successfully")
        
        # Test 1: Explain a metric result
        print("\n💡 Test 1: Explaining Revenue Growth")
        test_rag_context = "\n".join([ctx['text'][:100] for ctx in all_contexts[:3]]) if all_contexts else "Revenue data extracted."
        explanation1 = narrator.explain(
            user_question="What is the revenue growth?",
            rag_context=test_rag_context,
            computed_result="Revenue Growth: +8.3% (calculated from verified data)",
            charts=["revenue_trend.png"] if charts else None
        )
        print(f"\n📊 Gemini Explanation:\n   {explanation1}")
        
        # Test 2: Advisory question
        print("\n💡 Test 2: Advisory Question")
        explanation2 = narrator.explain(
            user_question="How can I improve cash flow?",
            rag_context=test_rag_context,
            computed_result="Current metrics: Revenue growing, cash runway 7 months",
            charts=None
        )
        print(f"\n📊 Gemini Advisory:\n   {explanation2}")
        ...
        print("\n💡 Test 3: Low Confidence Scenario")
        explanation3 = narrator.explain(
            user_question="What is the profit margin?",
            rag_context="Limited profit data available.",
            computed_result="Profit Margin: Unable to calculate (missing COGS data). Confidence: 30%",
            charts=None
        )
        print(f"\n📊 Gemini Low-Confidence Response:\n   {explanation3}")
        print("\n✅ LLM Narration: WORKING")

    except ImportError:
        print("\n   ❌ LLM Narrator module not found (src/ai/llm_narrator.py)")
    except Exception as e:
        print(f"\n   ❌ LLM Test Error: {e}")
    ...
    # ... (Summary generation as defined in original script)
